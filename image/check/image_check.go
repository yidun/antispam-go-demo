...
const (
	apiUrl     = "http://as.dun.163.com/v5/image/check"
	version    = "v5"
			...
			if resultMap, ok := result.(map[string]interface{}); ok {
				if resultMap["antispam"] != nil {
					...
					if status == 2 {
						...
						fmt.Printf("taskId: %s, status: %d, name: %s, suggestion: %d", taskId, status, name, suggestion)
						...
						fmt.Printf("图片检测失败, taskId: %s, failureReason: %d, name: %s", taskId, failureReason, name)
					}
				}
				if resultMap["ocr"] != nil {
					ocrMap, _ := resultMap["ocr"].(map[string]interface{})
				...
				if resultMap["face"] != nil {
					faceMap, _ := resultMap["face"].(map[string]interface{})
				...
				if resultMap["quality"] != nil {
					qualityMap, _ := resultMap["quality"].(map[string]interface{})
