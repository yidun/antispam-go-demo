...
@Author : yidun_dev
@Date : 2020-10-15
@File : liveaudio_querymonitor.go
@Version : 1.0
@Golang : 1.13.5
...
const (
	apiUrl     = "http://as-liveaudio.dun.163.com/v1/liveaudio/query/monitor"
	version    = "v1.0"
...
func main() {
	...
	if code == 200 {
		...
		if status == 0 {
			monitors, _ := result["monitors"].([]interface{})
			for _, monitor := range monitors {
				if monitorMap, ok := monitor.(map[string]interface{}); ok {
					_, _ = monitorMap["action"].(json.Number).Int64()
					_, _ = monitorMap["actionTime"].(json.Number).Int64()
					_, _ = monitorMap["spamType"].(json.Number).Int64()
					_, _ = monitorMap["spamDetail"].(string)
				}
			}
			fmt.Printf("直播人审结果：%s", monitors)
		} else if status == 20 {
			fmt.Printf("数据过期")
		} else if status == 30 {
			fmt.Printf("数据不存在")
